...
import top.anets.file.strategy.impl.minio.model.OssFile;
...
//    private final MinioConfig minioConfig;
    ...
    public OssFile putObject(InputStream inputStream, String bucketName, String originalFileName) {
        ...
        try {
            if (ObjectUtils.isEmpty(bucketName)) {
                bucketName = fileProperties.getMinIo().getBucket();
            }
     ...
     * 查询路径信息
     */
    public Iterable<Result<Item>> listObjects(String bucketName,String path, boolean recursive){
        Iterable<Result<Item>> results = minioClient.listObjects(ListObjectsArgs.builder().bucket(bucketName).prefix(path).recursive(recursive).build());
        return results;
    }
    ...
    public void initDefaultBucket() {
        String defaultBucketName = fileProperties.getMinIo().getBucket();
        if (bucketExists(defaultBucketName)) {
            log.info("默认存储桶：defaultBucketName已存在");
        } else {
            log.info("创建默认存储桶：defaultBucketName");
            makeBucket(fileProperties.getMinIo().getBucket());
        }
    }
    ...
    @SneakyThrows
    public OssFile composeObject(String originBucketName,String path, String targetBucketName, String objectName) {
        Iterable<Result<Item>> results = listObjects(originBucketName,path, true);
        List<String> objectNameList = new ArrayList<>();
        for (Result<Item> result : results) {
            Item item = result.get();
            objectNameList.add(item.objectName());
